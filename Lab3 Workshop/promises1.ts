...
const getAmount = (data: any) => new Promise<string>( (resolve) => {
  const convertTo = readline.question('Enter amount of HK$ to be converted: ');
  data.prop="amount";
  data.amount=convertTo;
  console.log(data);
  resolve(data);
});
...
const printObject = (data: any) => new Promise<any>( resolve => {
 ...
 // const str = JSON.stringify(data, null, indent);
  ...
  let convertAmount=rate[0]*parseInt(data.amount);     
